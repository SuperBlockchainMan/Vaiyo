...
#    end
  ...
  # Account verification after update profile information
  def verify_account_info
    @response = Hash.new
    pending_verification = Verification.where(:token => params[:token]).first
    ...
    if pending_verification 
      ...
      @response['msg'] = 'Token is invalid. Kindly try to update your information again.'
      @response['status'] = false
    end

    render ("users/verification"), @response
  end
